...
public class UpgradeAbilitySlotPanelTest {
  ...
  public void buttonTextTest() {
    slotPanel.updatePanel(new AbilitySlotClickEvent(0, BattleMove.PHYSICAL, new AbilitySlot(BattleMove.PHYSICAL, 0)));
    Assert.assertEquals("Level zero ability slot event should show 'Unlock' for the button text", "Unlock", slotPanel.getButtonText());
    slotPanel.updatePanel(new AbilitySlotClickEvent(1, BattleMove.PHYSICAL, new AbilitySlot(BattleMove.PHYSICAL, 1)));
    Assert.assertEquals("Level one ability slot event should show 'Upgrade' for the button text", "Upgrade", slotPanel.getButtonText());
  }
  ...
  public void panelVisibilityTest() {
    ...
    slotPanel.updatePanel(new AbilitySlotClickEvent(100, BattleMove.PHYSICAL, new AbilitySlot(BattleMove.PHYSICAL, 100)));
